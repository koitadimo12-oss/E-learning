...
const items: { to: string; label: string; emoji: string }[] = [
  { to: "/formateur/dashboard", label: "Dashboard", emoji: "🏠" },
  { to: "/formateur/my-courses", label: "Mes cours", emoji: "📚" },
  { to: "/formateur/create-course", label: "Créer un cours", emoji: "➕" },
  { to: "/formateur/modules", label: "Modules & Contenu", emoji: "🧩" },
  { to: "/formateur/quiz-builder", label: "Quiz Builder", emoji: "🧠" },
  { to: "/formateur/students", label: "Étudiants", emoji: "👥" },
  { to: "/formateur/stats", label: "Statistiques", emoji: "📊" },
  { to: "/formateur/profil", label: "Profil", emoji: "👤" },
];

const navCls = ({ isActive }: { isActive: boolean }) =>
  `flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium transition ${
    isActive ? "bg-orange-500 text-white shadow" : "text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"
  }`;
    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <aside className="flex w-72 shrink-0 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-200 p-4 dark:border-slate-800">
          <p className="font-bold text-orange-600 dark:text-orange-400">Espace formateur</p>
          <p className="truncate text-sm text-slate-500 dark:text-slate-400">{user?.name}</p>
          <p className="mt-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Pilotage des cours</p>
        </div>
        <nav className="flex flex-1 flex-col gap-1 p-3">
          {items.map((item) => (
            <NavLink key={item.to} to={item.to} className={navCls} end={item.to === "/formateur/dashboard"}>
      ...
      <main className="flex-1 overflow-auto p-6">
        <div className="mx-auto max-w-6xl">
          <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <p className="font-semibold">Bienvenue {user?.name}, gérez vos contenus et vos statistiques.</p>
          </div>
          <Outlet />
        </div>
      </main>
