...
export default function InscriptionFormateur() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!isUrl(portfolio) || !isUrl(linkedin) || !isUrl(github)) {
      setError(
        "Portfolio, LinkedIn et GitHub doivent être des liens valides."
      );
      return;
    }
        ...
        <div className="w-full lg:w-1/2 bg-white dark:bg-slate-900 p-8 flex items-center">
          <div className="w-full max-w-md mx-auto">
            ...
            <form className="mt-6 space-y-3" onSubmit={handleSubmit}>
              ...
              <ChampSaisie label="Mot de passe" type="password" name="password" value={password} onChange={(e: ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)} />
              <ChampSaisie label="Spécialité" type="text" name="specialite" value={specialite} onChange={(e: ChangeEvent<HTMLInputElement>) => setSpecialite(e.target.value)} />
              <ChampSaisie label="Expérience" type="text" name="experience" value={experience} onChange={(e: ChangeEvent<HTMLInputElement>) => setExperience(e.target.value)} />
